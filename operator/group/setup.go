...
func SetupController(mgr ctrl.Manager) error {
	name := strings.ToLower(miniov1alpha1.GroupGroupKind)
...
func createReconciler(mgr ctrl.Manager, name string, recorder event.Recorder, c managed.ExternalConnecter, creationGracePeriod time.Duration) *managed.Reconciler {
	...
	return managed.NewReconciler(mgr,
		resource.ManagedKind(miniov1alpha1.GroupGroupVersionKind),
...
func SetupWebhook(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr).
		...
		WithValidator(&Validator{
			log: mgr.GetLogger().WithName("webhook").WithName(strings.ToLower(miniov1alpha1.GroupKind)),
		}).
		Complete()
}
