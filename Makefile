...
webhook-debug:
	...
	yq e "del(.webhooks[3].clientConfig.service) | .webhooks[3].clientConfig.caBundle |= \"$$cabundle\" | .webhooks[3].clientConfig.url |= \"https://$$HOSTIP:9443//validate-minio-crossplane-io-v1alpha1-group\"" - | \
	yq e "del(.webhooks[4].clientConfig.service) | .webhooks[4].clientConfig.caBundle |= \"$$cabundle\" | .webhooks[4].clientConfig.url |= \"https://$$HOSTIP:9443//validate-minio-crossplane-io-v1alpha1-identityprovider\"" - | \
	kubectl apply -f - && \
